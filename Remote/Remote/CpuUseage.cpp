...
BOOL CCpuUsage::Init()
{
	...
	PDH_STATUS pdh_status = PdhAddCounter(m_hQuery, szCounterName, (DWORD) m_pCounterStruct, &(m_pCounterStruct->hCounter));    //统计感兴趣的系统信息时，必须先将对应的计数器添加进来
...
int CCpuUsage::GetUsage()
{
	...
	if (ERROR_SUCCESS != PdhGetFormattedCounterValue(                 //得到数据获得当前计数器的值
